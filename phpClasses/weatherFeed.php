 ...
 */
class weatherFeed extends feedDashboard
{
    /**
     ...
     */
    public function createWeatherFeed($apiKey, $location = "Dresden", $language = "en", $isCelsius = true, $isKpH = true, $isMM = true) {
        $htmlOutput = "";

        //get weather live and forecast (3-day)
        $weatherJSON = $this->callAPI("GET", "https://api.weatherapi.com/v1/forecast.json?key=$apiKey&q=$location&days=3&lang=$language");
        $weather = json_decode($weatherJSON, true);

        //check JSON for errors
        if (isset($weather["error"])) {
            $htmlOutput .= "<script> console.warn('Weather API ERROR " . $weather['error']["code"] . ": " . $weather['error']["message"] . "'); </script>";
            $htmlOutput .= $this->emptyFeed("Error while loading. <br> Please contact <a href='mailto:dashboard@tequilian.de?subject=Error on dashboard weather: " . $weather['error']["code"] . ": " . $weather['error']["message"] . "'>dashboard@tequilian.de</a>");
            return $htmlOutput;
        }
